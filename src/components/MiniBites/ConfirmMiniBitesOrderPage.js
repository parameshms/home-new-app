...
import { setMiniBitesOrderData } from "../../constants/miniBitesSlice";
import AddedItemMiniBites from "./AddItemsMiniBites";
import { placeMiniBitesOrderApi } from "../../apis/miniBitesApi";

const ConfirmMiniBitesOrderPage = () => {
  const [placedOrderData, setPlacedOrderData] = useState(null);
  const [totalPrice, setTotalPrice] = useState(0);
  const orderData = useSelector(
    (store) => store.miniBitesItems.miniBitesOrderData
  );
  ...
  const handleAddItem = (itemName) => {
    ...
    dispatch(setMiniBitesOrderData(updatedData));
  };
  ...
  const handlePlaceOrder = (data) => {
    const fetchData = async () => {
      ...
      try {
        const result = await placeMiniBitesOrderApi(payload, hotel_name);
...
export default ConfirmMiniBitesOrderPage;
