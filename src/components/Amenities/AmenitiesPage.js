...
import { amenitiesRequestApi } from "../../apis/amenitiesApi";
...
const AmenitiesPage = () => {
  ...
  const requestType = async (service) => {
    try {
      const data = await requestTypesApi(hotel_name, "hotel_amenities");
      setServiceType(data);
    } catch (error) {
  ...
  const requestAdd = async (requestId) => {
    ...
    console.log(payload, "Amenities")
    try {
      await amenitiesRequestApi(payload, hotel_name);
    } catch (error) {
    ...
    <div className="flex min-h-full flex-1 flex-col justify-center font-poppins">
      <div className="sm:mx-auto sm:w-full sm:max-w-sm mt-6 p-2">
        <div className="min-h-full font-poppins tracking-wide flex gap-6 p-2">
          ...
          />
          <h2 className="font-semibold text-[18px]">Hotel Amenities</h2>
        </div>
